...
public extension SCardPCI {
    init(pdwProtocol: SCardProtocol) throws (SCardError) {
        try self.init(pdwActiveProtocol: pdwProtocol.uInt32)
    }
}

extension SCardPCI {
    init(pdwActiveProtocol: UInt32) throws (SCardError) {
        switch pdwActiveProtocol {
        case SCardProtocol.t0.uInt32:
            self = try .init(
                protocol: .required(with: UInt32(cg_rgSCardT0Pci.dwProtocol)),
                length: cg_rgSCardT0Pci.cbPciLength
            )
        case SCardProtocol.t1.uInt32:
            self = try .init(
                protocol: .required(with: UInt32(cg_rgSCardT1Pci.dwProtocol)),
                length: cg_rgSCardT1Pci.cbPciLength
            )
        case SCardProtocol.raw.uInt32:
            self = try .init(
                protocol: .required(with: UInt32(cg_rgSCardRawPci.dwProtocol)),
    ...
    init(IO_REQUEST: CSCARD_IO_REQUEST) throws (SCardError) {
        self = try .init(
            protocol: .required(with: UInt32(IO_REQUEST.dwProtocol)),
